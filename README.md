...
## Апдейт 23 09 2026
Додано надійний варіант автоматичного проходження капчі (опціонально) за допомогою сервісу 2captcha.com. Цей сервіс платний, але плата за кожне розв'язання капчі при автоматичному надсиланні відгуку на вакансію мінімальна. 
...
├── solver.js            # Автоматичне розв'язання reCAPTCHA v2 (Vosk + audio challenge)
├── download-model.js    # Завантаження моделі Vosk для розпізнавання мови
├── vosk_models/         # Локальні моделі Vosk (у .gitignore)
...
### Автопроходження reCAPTCHA (опціонально)

Для повної автоматизації в налаштуваннях є вкладка **«reCAPTCHA»**, де можна увімкнути **автоматичне розв'язання капчі**:

1. Увімкніть перемикач **«Автоматичне розв'язання reCAPTCHA»**.
...
- Якщо checkbox не проходить одразу — переходить в **аудіорежим**, завантажує звукове завдання, розпізнає текст через локальну модель **Vosk** (маленька, ~40 МБ, повністю офлайн) і вводить відповідь.
- Після успішного розв'язання автоматично натискає **«Відправити»**.
- Статус вакансії змінюється на «Є заявка» без участі користувача.

> **Примітка:** Google періодично ускладнює аудіозавдання або блокує автоматичне проходження. Якщо кілька спроб поспіль не вдаються, система збільшує інтервал між ними та автоматично пропускає антибот-контент. При повному невдалому розв'язанні вікно лишається відкритим для ручного проходження.
...
| POST | `/api/settings/captcha` | Зберегти налаштування reCAPTCHA solver |
...
Форма захищена reCAPTCHA. За замовчуванням програма заповнює все, окрім капчі — її проходить користувач і натискає «Відправити». Якщо увімкнути **автопроходження reCAPTCHA** у налаштуваннях, програма розв'язує капчу автоматично й сама натискає «Відправити» — заявка надсилається без участі користувача.
